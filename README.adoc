...
workspace. Add a registration file that hands the registry to this extension:

[source,js]
.<workspace>/.asciidoctor/lib/ldl.js
----
const ldl = require('@openpowershift/asciidoctor-ldl')
...
Install the packages in the workspace so they resolve, then reload the window:

[source,console]
----
$ npm install @openpowershift/asciidoctor-ldl @openpowershift/logic-diagram-language
$ npm install @resvg/resvg-js        # for PNG previews
----

`[ldl]` blocks and `ldl::` macros then render in the live preview. This needs a
build of Asciidoctor.js that supports asynchronous extensions (Asciidoctor.js 4);
older bundled versions may not render the diagrams.
